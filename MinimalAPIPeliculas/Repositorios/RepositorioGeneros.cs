...
namespace MinimalAPIPeliculas.Repositorios
{
    public class RepositorioGeneros : IRepositorioGeneros
    {
        ...
        public async Task<int> CrearGenero(Genero genero)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
                var id= await conexion.QuerySingleAsync<int>("Generos_CrearGenero", new {genero.Nombre}, commandType:CommandType.StoredProcedure);
                genero.Id = id;
                return id;
            }
        ...
        public async Task<bool> Existe(int id)
        {
            using(var conexion = new SqlConnection(connectionString))
            {
                var existe = await conexion.QuerySingleAsync<bool>("Generos_Existe", new { id },commandType:CommandType.StoredProcedure);
                return existe;
            }
        }
        ...
        //obtner genero por id
        public async Task<Genero?> ObtenerXId(int id)
        {
            using (var conexion = new SqlConnection(connectionString))
            {
